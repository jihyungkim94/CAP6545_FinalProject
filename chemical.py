...
def main(subontology='mf', nb_epoch=15, gram_len=1, batch_size=64, run=1):

    tf.keras.backend.clear_session()


    model_path = 'models/chemical_' + subontology + '_e' + str(nb_epoch) \
                 + '_b' + str(batch_size) + '_n' + str(gram_len) + '_r' + str(run)
    ...
    train_generator = DataGenerator(train_data, train_labels, batch_size, nb_classes, vocab, gram_len)
    valid_generator = DataGenerator(val_data, val_labels, batch_size, nb_classes, vocab, gram_len)
    test_generator = DataGenerator(test_data, test_labels, batch_size, nb_classes, vocab, gram_len)
...
class DataGenerator(Sequence):
    ...
    def __init__(self, inputs, targets, batch_size, num_outputs, vocab, gram_len, shuffle=False):
        self.start = 0
        self.inputs = inputs
...
def train_model(model_path, nb_classes, gram_len, vocab_len, train_generator, valid_generator, nb_epoch):
